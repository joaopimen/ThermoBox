...
def get_unifac_parameters(selected_components):
    ...
    for comp in selected_components:
        if comp in unifac_lle_database.unifac_molecules:
            # Count occurrences of functional groups in the molecule
            group_counts = Counter(unifac_lle_database.unifac_molecules[comp])
    ...
    for group in sorted_groups:
        data = unifac_lle_database.unifac_groups[group]
        unifac_params["Rk"].append(data["Rk"])
        unifac_params["Qk"].append(data["Qk"])

    # Retrieve interaction parameters
    interaction_params = get_interaction_parameters(sorted_groups, unifac_lle_database.list_Interactions)
...
def get_interaction_parameters(selected_groups, list_Interactions):
    ...
    main_groups = [unifac_lle_database.unifac_groups[group]["main_group"] for group in selected_groups]
    ...
    for i, group_i in enumerate(selected_groups):
        for j, group_j in enumerate(selected_groups):
            main_i = unifac_lle_database.unifac_groups[group_i]["main_group"]
            main_j = unifac_lle_database.unifac_groups[group_j]["main_group"]
